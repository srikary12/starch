// Package secret stores the API key in the desktop's Secret Service.
//
// This is the Linux counterpart of the macOS Keychain, and the same rule
// applies: it is the only place a key is ever stored. It never goes into the
...
// file: a key sitting in a dotfile would quietly break the guarantee the
// README makes, and a machine with no keyring is better told so.
//
